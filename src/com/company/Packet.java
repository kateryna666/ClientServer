...
public class Packet {
    ...
    private boolean checkCRC1(){
        byte[] header = ByteBuffer.allocate(14).put(this.bMagic).put(this.bSrc).putLong(bPktId).putInt(wLen).array();
        return this.wCrc16==genCRC(header);
    }

    private boolean checkCRC2(){
        byte[] massage = ByteBuffer.allocate(wLen).put(this.bMsg.getMassage()).array();
        return this.wCrc16_2==genCRC(massage);
    }
