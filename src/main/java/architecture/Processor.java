...
public class Processor extends Thread{
    final static BlockingQueue<AbstractMap.SimpleEntry<Integer,Packet>> queueProcessor = new ArrayBlockingQueue<>(ServerTCP.CAPACITY);
    ...
    public void process() {
            try {
                int i = 0;
                while (!shutdown) {
                    if(!Decryptor.queuePackets.isEmpty()) {
                        AbstractMap.SimpleEntry<Integer, Packet> ip;
                        synchronized (Decryptor.queuePackets) {
                            ip = Decryptor.queuePackets.poll(10L, TimeUnit.SECONDS);
                        }
                        if (ip != null) {
                            int userId = ip.getKey();
                            Packet packet = ip.getValue();
                            ...
                            queueProcessor.put(new AbstractMap.SimpleEntry<>(userId, answer(packet)));
                        }
                    }
